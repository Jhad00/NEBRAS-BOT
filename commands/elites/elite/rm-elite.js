...
export default {
  cmd: "نخبة_ازل",
  description: "ازالة عضو من نظام النخبة",
  aliases: ["elite_rm", "سحب_النخبة"],
  category: "نخبة",
  do: async (wa, msg, args) => {
    try {
      ...
      // random responses for dev removal attempts (same as add)
      const devResponses = [
      ...
      // resolve invoker identity once, needed to detect "dev removing himself" case
      ...
      if (mentionedJids && mentionedJids.length > 0) {
        ...
        // check if user exists in elite before removing
        const currentElite = await wa.elites() || [];
        const isElite = currentElite.includes(lid);
        
        if (!isElite) {
          await wa.react("ℹ️");
          return wa.sendMessage(chatJid, { 
            text: `ℹ️ @${phone} غير موجود في قائمة النخبة.`,
            mentions: [jid]
          });
        }
        
        const result = await wa.removeElite([lid]);
        if (result && result[0]?.success) {
          await wa.react("✅");
          return wa.sendMessage(chatJid, { 
            text: `👑 تمت إزالة @${phone} من قائمة النخبة بنجاح.`,
            mentions: [jid]
          });
        }
        await wa.react("⚠️");
        return wa.sendMessage(chatJid, { text: "❌ فشل إزالة العضو." });
      }
      ...
      // 4. if private chat with bot, remove the user directly
      if (!isGroup) {
        const jid = chatJid;
        const phone = jid.split("@")[0];
...
📌 *طرق الإزالة:*
• رد على رسالة العضو بـ \`${prefix}نخبة_ازل\`
• اكتب: \`${prefix}نخبة_ازل +123456789\`
• منشن العضو: \`${prefix}نخبة_ازل @محمد\`
• في الخاص: استخدم الأمر بدون وسائط لإزالة نفسك`;
      ...
      return wa.sendMessage(chatJid, { 
        text: `⚠️ *طريقة الاستخدام:*\n\n` +
              `• \`${prefix}نخبة_ازل +123456789\` - برقم الهاتف\n` +
              `• منشن العضو: \`${prefix}نخبة_ازل @محمد\`\n` +
              `• رد على رسالة العضو بـ \`${prefix}نخبة_ازل\`\n` +
              `• في الخاص: استخدم الأمر بدون وسائط لإزالة نفسك`
      });
      ...
      console.error("نخبة_ازل error:", error);
      await wa.react("⚠️");
